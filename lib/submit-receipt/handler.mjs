...
export const handler = async (event) => {
  ...
  const shopperUsername = event["shopper-username"]; //ts syntax
  ...
  const insertItem = (item) => {
    return new Promise((resolve, reject) => {
      const insertQuery =
        "INSERT INTO Item (uuid, `receipt-uuid`, name, price, quantity) VALUES (?, ?, ?, ?, ?);";

      const itemUUID = uuidv4();

      pool.query(
        insertQuery,
        [itemUUID, receiptUUID, item.name, item.price, item.quantity],
        (error, insertResult) => {
          if (error) {
            reject(new Error("Database error: " + error.sqlMessage));
          } else {
