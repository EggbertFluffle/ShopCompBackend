...
export const handler = async (event) => {
  ...
  if (!event["item-quantity"]) throw new Error("Missing item-quantity");
  ...
  const addItemToList = (itemName, itemQuantity, listUUID) => {
    return new Promise((resolve, reject) => {
      const insertQuery = `
          INSERT INTO \`Shopping List Item\` (uuid, name, quantity, \`list-uuid\`)
          VALUES (?, ?, ?, ?);
      `;
      pool.query(insertQuery, [uuidv4(), itemName, itemQuantity, listUUID], (error, results) => {
        if (error) {
          reject(new Error("Database error: " + error));
        } else {
          resolve(results);
  ...
  try {
    ...
    await addItemToList(event["item-name"], event["item-quantity"], event["shopping-list-uuid"]);
